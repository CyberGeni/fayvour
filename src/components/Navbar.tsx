  ...
  return (
    <div className={`bg-[#EBEAED] overflow-hidden text-xl flex items-center justify-between font-athletics px-[5%] sm:px-[7.5%] md:px-[10%] py-8 md:py-12 ${isActive ? "overflow-x-hidden" : ""}`}>
      ...
      <nav className={`text-[#272727]/75 text-lg font-medium sm:space-x-6 transition-all ${isActive ? "bg-[#EBEAED] sm:bg-transparent flex flex-col sm:flex-row justify-center fixed sm:relative space-x-0 w-screen sm:w-fit text-center left-0 sm:left-[unset] top-20 sm:top-[unset] space-y-4 sm:space-y-0 h-48 sm:h-fit z-10 overflow-x-hidden" : "fixed -top-24 sm:relative sm:top-0"}`}>
        <NavLink onClick={() => handleClick()} to="/" className="animated-link">ABOUT</NavLink>
        <a onClick={() => handleClick()} href="#work" className='animated-link'>WORK</a>
        <a className='animated-link' href="https://drive.google.com/file/d/1WYtvZb_a8wa-sk9EG2-WyOBkxm02kYdv/view?usp=drivesdk " target='_blank'>RESUME</a>
      </nav>
